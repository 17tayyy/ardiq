...
--       9 result key, 10 results index, 11 reset (1 = drop any stored result).
...
local now           = ARGV[8]
local result_key    = ARGV[9]
local results_index = ARGV[10]
local reset         = tonumber(ARGV[11])
...
-- Callers that reuse an id on purpose (a unique task) start a fresh run: the
-- previous result must go, or status() would report `complete` and result()
-- would hand back the old run's value for a task that has not run yet.
if reset == 1 then
